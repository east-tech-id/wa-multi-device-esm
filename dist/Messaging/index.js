...
export const sendTextMessage = async ({ sessionId, to, text = "", isGroup = false, ...props }) => {
...
export const sendImage = async ({ sessionId, to, text = "", isGroup = false, media, ...props }) => {
    const session = getSession(sessionId);
    if (!session)
        throw new WhatsappError(Messages.sessionNotFound(sessionId));
    to = phoneToJid({ to, isGroup });
...
export const sendVideo = async ({ sessionId, to, text = "", isGroup = false, media, ...props }) => {
    const session = getSession(sessionId);
    if (!session)
        throw new WhatsappError(Messages.sessionNotFound(sessionId));
    to = phoneToJid({ to, isGroup });
...
export const sendDocument = async ({ sessionId, to, text = "", isGroup = false, media, filename, ...props }) => {
    const session = getSession(sessionId);
    if (!session)
        throw new WhatsappError(Messages.sessionNotFound(sessionId));
    to = phoneToJid({ to, isGroup });
...
export const sendVoiceNote = async ({ sessionId, to, isGroup = false, media, ...props }) => {
    const session = getSession(sessionId);
    if (!session)
        throw new WhatsappError(Messages.sessionNotFound(sessionId));
    to = phoneToJid({ to, isGroup });
...
export const sendSticker = async ({ sessionId, to, isGroup, media, ...props }) => {
    const session = getSession(sessionId);
    if (!session)
        throw new WhatsappError(Messages.sessionNotFound(sessionId));
    to = phoneToJid({ to, isGroup });
 ...
 */
export const sendTyping = async ({ sessionId, to, duration = 1000, isGroup = false, }) => {
    const oldPhone = to;
    to = phoneToJid({ to, isGroup });
    const session = getSession(sessionId);
    if (!session)
 ...
 * @param duration - Duration in miliseconds typing effect will appear
 */
export const readMessage = async ({ sessionId, key }) => {
    const session = getSession(sessionId);
    if (!session)
        throw new WhatsappError(Messages.sessionNotFound(sessionId));
